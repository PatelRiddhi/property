...
?>   
                  <tr>
                    ...
                    <td><a class="btn btn-primary" id="approval" name="action" title="" data-id="<?php echo $row['id']; ?>" ><?php if($row['status']==0){echo "approve";}
                          if($row['status']==1){ echo "disapprove";} ?></a></td>
...
<script type="text/javascript">
  $(document).ready(function(){
    $('a[title]').each(function(index,ele){
     $(ele).click(function(){
      var id=$(this).attr('data-id');
      alert(id);
      $.ajax({
                url: '<?php echo base_url('admin/agencies/approval');?>',
                type: 'POST',
                data: {"id":id},
